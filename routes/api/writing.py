...
def new_character(request: Request):
    character = secrets.choice(
        open("handwriting/labels_fixed.txt", encoding="utf-8").readlines()
    ).strip()
...
async def writing(request: Request, _: int = Depends(get_authenticated_user)):
    """
    ...
    return templates.TemplateResponse(
        "writing.html",
        {"request": request, "character": character, "explanation": explanation},
    )


@router.get("/reset", response_class=HTMLResponse)
async def writing_reset(request: Request, _: int = Depends(get_authenticated_user)):
    """
    Display the writing page
