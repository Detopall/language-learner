...
def get_quiz():
    ...
    prompt = "日本語の穴埋め問題を1つ作成してください。返答は必ず次のJSON形式で出力してください。{'question': <空欄を含む日本語の文>, 'choices': [<選択肢1>, <選択肢2>, <選択肢3>, <選択肢4>]} 文、選択肢、正解はすべて日本語（漢字・ひらがな・カタカナ）で書いてください。ローマ字は使わないでください。"
...
def get_correct_answer(question: str, choices: list):
    ...
    prompt = (
        ...
        + " and the choices: "
        + str(choices)
        + " which one is the correct answer? Respond with the number of the correct answer, starting from 1. Make sure the response is a number and is in the range of the choices. Do not respond with anything else."
    )
...
def auto_generate_quiz_until_stable(max_iterations=3):
    ...
    for _ in range(max_iterations):
        if (
            ...
            or question == ""
            or any(choice == "" for choice in choices)
        ):
            question, choices = get_quiz()
        else:
            break
...
async def quizzes(request: Request, _: int = Depends(get_authenticated_user)):
    """
    Display the quizzes page

    Args:
            request (Request): The request object
            _ (int, optional): The user ID. Defaults to Depends(get_authenticated_user).
...
async def create_questions(request: Request, _: int = Depends(get_authenticated_user)):
    """
    Display the quizzes page
    ...
    return templates.TemplateResponse(
        "quiz_form.html", {"request": request, "question": question, "choices": choices}
    )
...
async def check_answer(
    request: Request,
    _: int = Depends(get_authenticated_user),
    answer: str = Form(...),
):
    """
    Display the quizzes page
    ...
    feedback, color = (
        ("Correct", "green")
        if (answer == choices[int(correct_index) - 1])
        else (
            "Incorrect, the right answer is " + choices[int(correct_index) - 1],
            "red",
        )
    )
    return templates.TemplateResponse(
...
async def translate_questions(
    request: Request, _: int = Depends(get_authenticated_user)
):
    """
    Display the quizzes page
    ...
    translation = {"question": question_translation, "choices": choices_translation}

    return templates.TemplateResponse(
        "quiz_translation.html", {"request": request, "translation": translation}
    )
