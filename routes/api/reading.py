...
def generate_japanese_story(length="short", difficulty="easy"):
    ...
    prompt = (
        str(length_instruction)
        + " 日本語の物語を書いてください。"
        + str(difficulty_instruction)
        + " ください。返答は必ず次のJSON形式で出力してください。 {'title': <物語のタイトル>, 'story': <物語の本文（必ず空ではなく、内容を含めてください）>} タイトルと本文はどちらも日本語（漢字・ひらがな・カタカナ）で書いてください。ローマ字や英語は使わないでください。"
    )
...
async def reading(request: Request, _: int = Depends(get_authenticated_user)):
    """
...
async def create_story(
    ...
    for _ in range(max_tries):
        title, story = generate_japanese_story(
            length=story_length, difficulty=story_difficulty
        )
...
async def translate(request: Request, _: int = Depends(get_authenticated_user)):
    """
    Translate the story
    """
    ...
    title_translation, story_translation = auto_translate_until_stable(
        title
    ), auto_translate_until_stable(story)
